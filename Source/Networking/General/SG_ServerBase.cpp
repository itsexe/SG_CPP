...
void SG_ServerBase::SaveChar(const boost::shared_ptr<SG_ClientSession> Session)
{
		...
		MySQLQuery mysql_query(Session->SQLConn, "UPDATE Accounts SET Name = ?, Rank = ?, Level = ?, XP = ?, License = ?, Rupees = ?, Coins = ?, Questpoints = ?, ClanID = ?, CharType = ?, LastDailyCoins=FROM_UNIXTIME(?), age = ?, zoneid = ?, zoneinfo = ?, bio = ? WHERE id = ?");
